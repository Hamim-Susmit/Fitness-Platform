...
import { appleHealthAdapter } from "./adapters/appleHealth";
import { googleFitAdapter } from "./adapters/googleFit";
...
const PROVIDERS_WITH_MOBILE_SUPPORT: FitnessProvider[] = ["APPLE_HEALTH", "GOOGLE_FIT"];
...
export async function syncProviderForMember(
  ...
  if (!PROVIDERS_WITH_MOBILE_SUPPORT.includes(provider)) {
    throw new Error("provider_not_supported_on_mobile");
  }
